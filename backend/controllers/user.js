...
export const signIn = async (req, res, next) => {
    ...
    if (!email || !password) {
        return res
            ...
            .json({ message: ErrorMessage.NOT_FULL_CREDENTIALS });
    }
    ...
    if (user && isPasswordCorrect) {
        ...
        return res
            .status(HttpStatus.BAD_REQUEST)
            .json({ message: ErrorMessage.NOT_VALID_CREDENTIALS });
    }
};
...
export const signUp = async (req, res, next) => {
    ...
    if (Boolean(isEmailAlreadyExist)) {
        return res
            .status(HttpStatus.BAD_REQUEST)
            .json({ message: ErrorMessage.USER_ALREADY_EXIST });
    }
...
export const current = async (req, res) => {
    return res.status(HttpStatus.OK).json({ message: req.user });
};
